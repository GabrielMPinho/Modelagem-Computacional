...
8. **Formatação obrigatória:**
   ...
   - **Inclua uma resolução passo a passo em Python no final do `teoria.md`**, depois da teoria, exemplos, conclusões e referências. Organize-a como células de Jupyter na ordem de execução: bibliotecas, funções e variáveis, derivadas, gráfico inicial, ordem 0, ordem 1 e ordens seguintes.
   - Depois de cada célula, explique todas as linhas em uma tabela e mostre o output correspondente; para gráficos, descreva a saída visual. Calcule cada aproximação de forma acumulativa (`Tn = T(n-1) + novo termo`) e não arredonde `Tn` antes de calcular `En`.
   - **Ao responder se o aluno acertou uma questão, valide sempre o código atual do notebook**, lendo as células correspondentes e executando os cálculos quando possível. Considere a ordem real de execução das células e verifique se alguma variável foi herdada da questão anterior, sobrescrita ou usada antes de ser definida.
   - A resposta de validação deve separar: (a) o que está correto no código e nos resultados; (b) cada erro de lógica, fórmula, variável ou execução, com a célula onde ocorre; e (c) o que ainda não foi implementado. Não declare que o aluno acertou apenas porque os números coincidem com o gabarito.
